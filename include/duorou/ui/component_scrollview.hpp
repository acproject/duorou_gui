...
inline bool measure_node_scrollview(const ViewNode &node, ConstraintsF constraints,
  ...
  const auto axis = prop_as_string(node.props, "scroll_axis", "vertical");
  const bool allow_x = axis == "horizontal" || axis == "both";

  const auto inner_max_w = std::max(0.0f, constraints.max_w - padding * 2.0f);
  const auto inner = ConstraintsF{allow_x ? 1000000.0f : inner_max_w, 1000000.0f};
...
inline bool layout_children_scrollview(const ViewNode &node, RectF frame,
  ...
  const auto scroll_x = prop_as_float(node.props, "scroll_x", 0.0f);
  ...
  const auto inner = ConstraintsF{allow_x ? 1000000.0f : inner_w, 1000000.0f};
  ...
  const auto max_scroll_x =
      allow_x ? std::max(0.0f, static_cast<float>(content_w) - inner_w) : 0.0f;
  const auto max_scroll_y =
      allow_y ? std::max(0.0f, static_cast<float>(content_h) - inner_h) : 0.0f;

  const auto sx = clampf(static_cast<float>(scroll_x), 0.0f, max_scroll_x);
  const auto sy = clampf(static_cast<float>(scroll_y), 0.0f, max_scroll_y);

  out.scroll_x = sx;
  out.scroll_content_w = static_cast<float>(content_w);
  out.scroll_max_x = max_scroll_x;
  out.scroll_y = sy;
  out.scroll_content_h = static_cast<float>(content_h);
  out.scroll_max_y = max_scroll_y;

  const float x = inner_x - sx;
  float y = inner_y - sy;
  for (std::size_t i = 0; i < node.children.size(); ++i) {
    RectF child_frame{x, y, child_sizes[i].w, child_sizes[i].h};
...
inline bool emit_render_ops_scrollview(const ViewNode &v, const LayoutNode &l,
  ...
  if (!((l.scroll_max_x > 0.0f) || (l.scroll_max_y > 0.0f)) ||
      !(inner_h > 0.0f) || !(inner_w > 0.0f)) {
    return true;
  }

  const float bar_w = prop_as_float(v.props, "scrollbar_width", 6.0f);
  const float bar_h = prop_as_float(v.props, "scrollbar_height", 6.0f);
  ...
  if (l.scroll_max_x > 0.0f) {
    const float x = l.frame.x + padding + margin;
    const float y = l.frame.y + padding + std::max(0.0f, inner_h - bar_h - margin);
    const float w = std::max(0.0f, inner_w - margin * 2.0f);
    if (w > 0.0f) {
      const float thumb_w =
          std::max(12.0f, w * (inner_w / std::max(inner_w, l.scroll_content_w)));
      const float denom = std::max(1e-6f, l.scroll_max_x);
      const float t = clampf(l.scroll_x / denom, 0.0f, 1.0f);
      const float thumb_x = x + (w - thumb_w) * t;
      out.push_back(DrawRect{RectF{x, y, w, bar_h}, track});
      out.push_back(DrawRect{RectF{thumb_x, y, thumb_w, bar_h}, thumb});
    }
  }
  return true;
}
