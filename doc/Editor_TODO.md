...
- [x] 运行时 StyleManager：加载/切换主题/按层级合并
- [x] 组件应用样式：将计算结果绑定到渲染属性
- [x] 组件级样式覆盖（override 开关与覆盖作用域）
- [x] 状态样式验证：hover / active / disabled（含模拟入口）

### M4：依赖与继承系统（可解释的样式来源）
- [x] 实现主题继承：Dark 继承 Light（base 链合并）
- [x] 实现样式继承/依赖关系（按层级覆盖规则）
- [x] 生成可查询的“属性来源”信息（每个属性来自哪一层）

### M5：Style Inspector（核心调试能力）
- [x] 点击组件获取最终计算结果（所有可编辑属性）
- [x] 显示每个属性来自哪里（Global / Button / primary / hover 等）
- [x] 显示继承链与覆盖关系（被 override 的值可见）
- [x] 支持按属性搜索与按来源过滤

### M6：实时预览系统（所见即所得）
- [x] 预览支持多布局（不同页面/容器组合）
- [x] 预览支持多状态（hover/active/disabled/loading）
- [x] 支持缩放与滚动
- [x] 响应窗口变化（布局自适应验证）
...
- [x] 创建主题 / 删除主题 / 复制主题
- [x] 切换主题（即时刷新预览）
- [x] 主题继承可视化（base 链展示与冲突提示）

### M8：热加载 + 差异检测 + 局部刷新
- [x] 监听样式文件变化（文件监控）
- [x] 自动重新解析并生成 diff
- [x] 根据 diff 做局部刷新（最小影响范围更新）
- [x] 刷新稳定性验证（连续编辑/频繁保存/语法错误恢复）
...
- [x] 主题切换（Light/Dark/High-Contrast 至少 2 套可用）
- [x] hover/active/disabled 状态样式可复现且来源可追踪
- [x] 热加载保存后预览自动更新，且能说明 diff 与刷新范围
- [x] Style Inspector 能展示：最终值、来源、继承链、被覆盖项
